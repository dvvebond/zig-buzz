...
5. Run `pnpm --filter @buzz/media check` to verify that the TypeScript relay
   accepts every sanitized fixture while rejecting the three unsanitized
   fixtures that contain forbidden metadata.
6. Regenerate the Expo native project and run its platform test/build gate when
   changing the client-side sanitizer.
