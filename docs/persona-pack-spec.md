...
This is the format implemented by `packages/persona`.
...
> TypeScript desktop-host command or a future "Install Pack" UI button.
...
buzz-acp's `frameSystemPrompt()` in `packages/acp/src/harness.ts` prepends a `[System]` block to the **user message
...
| PF-6 | Per-subprocess environment injection: **Implemented.** `ManagedAgentService` validates named environment entries and passes them to Node's `spawn` without a shell. Reserved Buzz credential names cannot be overridden; operator environment values remain available to the selected runtime. | `apps/desktop-host/src/managed-agents.ts` |
