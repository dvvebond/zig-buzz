...
All replicas must share one PostgreSQL database. Delivery authority, replay admission, and endpoint quota reservation are transactional there, so replica count does not multiply the abuse ceiling. The gateway owns a scoped migration history under `apps/push-gateway/migrations`; it creates only the `push_gateway_*` authority tables and its migration-history table and never runs relay migrations.
